...
// defaults = your current constants
const SOCIALS_DEFAULT: Social[] = [
...
const FAQS_DEFAULT: FAQ[] = [
    { q: "Who is this course for?", a: "Anyone struggling with acid reflux or gut issues who wants a structured, natural plan with step-by-step guidance." },
    { q: "Do I need any prior knowledge?", a: "No. The course is beginner-friendly with simple frameworks, templates, and weekly actions." },
    { q: "How long do I get access?", a: "Lifetime access, including future updates and added resources." },
    { q: "Is there community or support?", a: "Yes. You get access to community Q&A and mentor support during the program windows." },
];

const FEATURES_DEFAULT: string[] = [
...
type Props = {
    ...
    faqs?: FAQ[];
    features?: string[];
    slides?: Slide[];           // for Carousel
    bannerVideoUrl?: string;    // for About video
    titleOverride?: string;     // headline override
...
const WaitList: React.FC<Props> = ({
    ...
    const chipText = React.useMemo(() => {
        if (!launchDate) return "Launching this September";
            ...
            <div className="flex flex-col items-center justify-center w-full px-2 sm:px-4">
                <div className="flex shadow-[0px_8px_16px_0px_#2A2A2A0D] py-2 px-3 sm:py-[10px] sm:px-[15px] h-[40px] sm:h-[43px] rounded-[31px] gap-2 sm:gap-[6px] max-w-full">
                    ...
                    <div className="flex flex-col items-center justify-center">
                        <div className="w-[22px] h-[22px] sm:w-[26px] sm:h-[26px] rounded-full bg-cover bg-center"
        ...
        <div className='w-screen h-[calc(100%-80px)] sm:h-[calc(100%-113px)] flex flex-col items-center overflow-x-hidden overflow-y-auto hide-scrollbar'>
            <div className='w-[95%] xl:w-[70%] h-auto py-[30px] flex flex-col'>
                ...
                <div className="w-full flex justify-center mt-6 sm:mt-10 md:mt-20">
                    <div className="inline-flex items-center h-9 sm:h-[35px] px-3 sm:px-[12px] py-1.5 sm:py-[6px] bg-white border border-[#ECECEC] rounded-[30px] gap-2 sm:gap-[6px] shadow-[0px_8px_16px_0px_#2A2A2A0D] max-w-[92%] sm:max-w-[70%] md:max-w-[50%]">
                        ...
                        <div className="flex items-center">
                            <span className="text-sm sm:text-[16px] font-medium whitespace-nowrap truncate max-w-[60vw] sm:max-w-[46vw] md:max-w-none" title="Launching this September">
                                {chipText}
                            </span>
                        </div>
                    </div>
                ...
                <div className="w-full flex justify-center mt-4 sm:mt-[15px] px-4">
                    <span className="font-semibold text-[46px] sm:text-[40px] md:text-[56px] leading-tight sm:leading-snug md:leading-[1.2] w-full sm:w-[80%] md:w-[65%] text-center">
                        {titleOverride ? (
                            // Convert <mark>...</mark> to <span class="text-[#0A5DBC]">...</span> and inject as HTML
                            <span
                                dangerouslySetInnerHTML={{
                                    __html: titleOverride.replace(
                                        /<mark[^>]*>(.*?)<\/mark>/g,
                                        '<span class="text-[#0A5DBC]">$1</span>'
                                    ),
                                }}
                            />
                        ) : (
                ...
                <div className="w-full flex justify-center mt-3 sm:mt-[15px] text-[#787878] px-4">
                    {subTextOverride ? (
                        <span
                            className="text-[16px] sm:text-[16px] md:text-[18px] font-normal text-center w-full sm:w-[70%] md:w-[50%] leading-relaxed"
                            dangerouslySetInnerHTML={{
                                __html: subTextOverride.replace(
                                    /<mark[^>]*>(.*?)<\/mark>/g,
                                    '<span class="text-[#0A5DBC]">$1</span>'
                                ),
                            }}
                        ...
                        <span className="text-[16px] sm:text-[16px] md:text-[18px] font-normal text-center w-full sm:w-[70%] md:w-[50%] leading-relaxed">
                            Be one of the first to get exclusive, early-access to this groundbreaking course.
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center">
                            <span className="relative inline-flex h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-yellow-400">
                                <img src="/t1.png" alt="" className="absolute h-[85%] w-[85%] inset-0 m-[3px] rounded-full object-cover ring-2 ring-white" />
                            </span>
                            <span className="relative inline-flex -ml-2 h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-cyan-400">
                                <img src="/t2.png" alt="" className="absolute h-[85%] w-[85%] inset-0 m-[3px] rounded-full object-cover ring-2 ring-white" />
                            </span>
                            <span className="relative inline-flex -ml-2 h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-red-500">
                                <img src="/t3.png" alt="" className="absolute h-[85%] w-[85%] inset-0 m-[3px] rounded-full object-cover ring-2 ring-white" />
                            </span>
                        </div>
                ...
                <div className='w-full flex justify-center mt-[60px]'>
                    <div className='w-full md:w-[80%] flex flex-col'>
                        <div className="inline-flex items-center h-8 sm:h-9 md:h-[35px] px-3 sm:px-[10px] md:px-[12px] py-1 sm:py-1.5 md:py-[6px] bg-white border border-[#ECECEC] rounded-[30px] gap-1.5 sm:gap-2 md:gap-[6px] shadow-[0px_8px_16px_0px_#2A2A2A0D] w-fit">
    ...
    <span className="block text-[24px] sm:text-[24px] md:text-[36px] font-semibold leading-snug sm:leading-normal md:leading-[1.3] sm:text-left">
      ...
      <span className="text-[#000]">without relying on medication</span>.
                        ...
                        <div className="mt-[30px] w-full bg-white rounded-[20px] px-[15px] pt-[25px] pb-[15px]">
                            ...
                            <div className="mt-[30px] w-full aspect-video">
                                <iframe
                                    ...
                                    src={bannerVideoUrl ?? "https://www.youtube.com/embed/fxx_E0ojKrc"}
                                    title="YouTube video"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4" role="list">
                                {features.map((text) => (
                                    <div
                                        key={text}
                                        role="listitem"
                                        className="flex items-start gap-3 p-4 sm:p-5 rounded-[20px] border border-[#ECECEC] text-[14px] sm:text-[15px] md:text-[16px] bg-white"
                                    >
                                        <IoCheckmark className='w-[18px] h-[18px] fill-[#0A5DBC] stroke-3 stroke-[#0A5DBC]' />
                                        <span className="text-[#000] font-[500] leading-relaxed">{text}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col mt-[30px] w-full">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-[15px] items-start" role="list">
                                {faqs.map(({ q, a }, i) => {
                                    const isOpen = openIdx === i;
                                    return (
                                        <div key={q} role="listitem" className="rounded-[20px] border border-[#ECECEC] bg-white p-0 overflow-hidden self-start">
                                            <button
                                                type="button"
                                            ...
                                            >
                                                <span className={`shrink-0 mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full border border-[#ECECEC] bg-white transition-transform ${isOpen ? 'rotate-90' : 'rotate-0'}`}>
                                                    <CiPlay1 className="h-4 w-4" />
                                                </span>
                                                <span className="text-[#000] text-[15px] sm:text-[16px] md:text-[16px] font-[600] leading-snug">{q}</span>
                                            </button>

                                            <div className={`transition-[max-height,opacity] duration-300 ease-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-0">
                                                    <p className="text-[#787878] text-[14px] sm:text-[15px] leading-relaxed">{a}</p>
                                                </div>
                                            </div>
                                        </div>
                                    );
                        ...
                        <div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mt-[15px]">
                            {socials.map(({ label, handle, icon, href }) => (
                                <a
                                    ...
                                    className="flex items-center gap-3 sm:gap-4 border border-[#ECECEC] rounded-[30px] px-4 py-3 sm:px-5 sm:py-4 hover:shadow-[0px_8px_16px_0px_#2A2A2A0D] transition"
                                >
